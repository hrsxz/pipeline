...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        # print(f"Entering Focus layer with shape: {x.shape}")  # 打印进入 Focus 层的形状
        # slice1 = x[..., ::2, ::2]
        # slice2 = x[..., 1::2, ::2]
        # slice3 = x[..., ::2, 1::2]
        # slice4 = x[..., 1::2, 1::2]
        ...
        # print(f"After concatenation in Focus layer, shape: {x.shape}")  # 打印拼接后的形状
        return self.conv(x)
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        # 确保所有张量在指定维度上大小一致
        min_shape = min([t.shape[2:] for t in x])  # 获取最小的宽高
        x = [
            (
                F.interpolate(t, size=min_shape, mode="bilinear", align_corners=False)
                if t.shape[2:] != min_shape
                else t
            )
            for t in x
        ]
