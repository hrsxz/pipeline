...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        try:
            ...
            ):  # win和linux有点区别 所以这里面代码稍微处理的内容多了点
        ...
        if self.rect:  # 矩形
            ...
            self.batch_shapes = np.ceil(
                np.array(shapes) * img_size / stride + pad
            ).astype(int)
        ...
        for i, file in pbar:
            ...
            if l is not None and l.shape[0]:
                ...
                ):  # duplicate rows 计算重复的
                    nd += 1  # print('WARNING: duplicate rows in %s' % self.label_files[i])  # duplicate rows
                if single_cls:
                    l[:, 0] = (
                        0  # force dataset into single-class mode 单个类别，设置其类别为0
                    )
                self.labels[i] = l
                nf += 1  # file found
    ...
    def __getitem__(self, index):
        ...
        # Convert
        # BGR to RGB, to 3x416x416 要满足pytorch的格式
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # 2.由于小图可能填充不满，所以还需要计算差异值，因为一会要更新坐标框标签
        padw = x1a - x1b
        padh = y1a - y1b
    ...
    # Concat/clip labels 坐标计算完之后物体框可能越界，调整坐标值，让他们都在大图中
    if len(labels4):
    ...
    # Augment 对整合的大图再进行随机旋转、平移、缩放、裁剪, 这样的仿射变换
    # 来增强数据
    # 使用的OPENCV, pytorch 自带的transform模块也可以完成同样的功能
